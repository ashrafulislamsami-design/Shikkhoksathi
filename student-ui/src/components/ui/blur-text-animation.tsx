  ...
  return (
    <div className={`flex items-center justify-start ${className}`}>
      <div className="text-left w-full">
        <Tag className={`${textColor} ${fontSize} ${fontFamily} ${fontWeight} ${leading} ${tracking} flex flex-wrap`}>
          {textWords.map((word, index) => {
            if (word.text === "\n" || word.text === "<br>" || word.text === "br-break") {
              return <div key={index} className="basis-full h-0"></div>;
            }
            return (
              <span
                key={index}
                className={`inline-block transition-all ${isAnimating ? 'opacity-100' : 'opacity-0'} ${word.className || ''}`}
                style={{
